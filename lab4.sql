...
INSERT INTO `ORDER`  VALUES( 116 ,99 ,"2021-09-17", 2 ,12 );
...
SELECT 
COUNT(T2.CUS_GENDER) AS CustomerCount ,
T2.CUS_GENDER 
 FROM 
( SELECT T1.CUS_ID, T1.CUS_GENDER,T1.ORD_AMOUNT, T1.CUS_NAME
FROM  (
		SELECT `ORDER`.*, CUSTOMER.CUS_GENDER, CUSTOMER.CUS_NAME FROM 
		`ORDER` INNER JOIN CUSTOMER WHERE `ORDER`.CUS_ID = CUSTOMER.CUS_ID
...
`ORDER`.pro_id = SUPPLIER_PRICING.PRO_ID
...
SELECT 
CATEGORY.CAT_ID, CATEGORY.CAT_NAME, MIN(T3.min_price) AS Min_Price FROM 
CATEGORY INNER JOIN 
(
